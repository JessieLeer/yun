  ...
  <div class='inner-main'>
	  <mt-header title="商品搜索" fixed class='header'>
		...
		<div v-if='histories.length > 0'>
			<mt-cell title="历史搜索" class='history'>
		...
		<div class='result' v-if='search.result.length > 0'>
		  <mt-cell to='/search1/result' v-for='(item,index) in search.result' v-bind:key='index' v-bind:title="item.name" id-link>
			  <mt-badge size="normal" color='#eee' class='badge' v-for='(badge,i) in item.badges' v-bind:key='i'>{{badge}}</mt-badge>
			</mt-cell>
		</div>
	</div>
</template>
