	  ...
	  <mt-header title="购物车" class='header'>
		...
		<div v-for='(item,index) in orders' v-bind:key='index' class='mt-10'>
			...
			<mt-cell label="修改" class='shopcar-cell'>
				<section class='price' slot='title' v-on:click='edit(item)'>
					<i class='price-now f-fsn f-fs1'>¥<span>{{item.price}}</span>/盒</i>
					<i class='price-old f-fsn f-fs1'>¥{{item.oldPrice}}/盒</i>
					<i class='order-count f-fsn'>x{{item.count}}</i>
		...
		<footer class='footer' v-else>
		  <section class='amount'>合计：<i class='f-fsn'>¥338.00</i></section>
			<mt-button type="default" v-bind:disabled='orders.length == 0 ? true : false' class='sub' size='small' v-on:click='go("/shopcar/result")'>提交({{orders.length}})</mt-button>
		</footer>
