		...
		<mt-popup v-model="typePickerShow" position="bottom" class='w-100'>
			<mt-picker :slots="slots" @change="onValuesChange" ref='typePicker' class='picker' showToolbar>
				<section class='w-100 f-cb'>
					...
					<mt-button @click="index(1)" class="sure f-fr" size='small'>确认</mt-button>
				</section>
			</mt-picker>
		</mt-popup>
		<mt-datetime-picker ref='monthPicker' v-model="search.month" v-bind:value='search.month' type="date" v-on:confirm='index(1)'></mt-datetime-picker>
	</div>	
</template>
